...
function action() {
  var userText = document.getElementById("user_text").value,
    ...
    userDirect = "";
  this.getAttribute("data-enc") === "decrypt"
    ? (userDirect = "-")
    : (userDirect = "+"); //check for user action
  if (checkData(userText, userKey)) {
    var keyArray = createKeyArray(userText, userKey), //get key array
...
function getEncryption(text, key, direction) {
  ...
  for (var i = 0; i < text.length; i++) {
    if (text[i].match(/[0-9!?@#$%^&*.,\s]/g)) {
      ...
      var index = eval(
        "alfa.indexOf(text[i].toUpperCase())" +
          direction +
          "alfa.indexOf(key[i].toUpperCase())"
      );
...
function getResult(arr, text) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (text[i] === text[i].toLowerCase())
      String(arr[i]) === arr[i]
        ? result.push(arr[i])
        : result.push(alfaLower[arr[i]]);
    else
      String(arr[i]) === arr[i]
        ? result.push(arr[i])
        : result.push(alfa[arr[i]]);
  }
  return result;
}
